...
void agregar_a_lista_favoritas(List *recetas_encontradas, int eleccion, List *favoritos)
{
    receta *aux = list_first(recetas_encontradas);
    int indice = 1;

    while (indice != eleccion)
    {
        list_pushBack(favoritos, aux);
        aux = list_next(recetas_encontradas);
        indice++;
    }
}

void recetas_favoritas(List *recetas_encontradas)
{
    if (recetas_encontradas == NULL)
    {
    ...
    printf("Seleccione la receta que desea añadir a su lista de favoritas con su referencia numerica\n");
    ...
    agregar_a_lista_favoritas(recetas_encontradas, eleccion, lista_favoritos);
    
}

void menu_recetas_favoritas(List *recetas_encontradas)
{
    char opcion;
    printf("Desea añadir una receta a su lista de favoritos? (S/N)\n");
    scanf(" %c", &opcion);
    if (opcion == 'S' || opcion == 's');
        recetas_favoritas(recetas_encontradas);
}
...
void buscar_recetas_posibles(Map *recetas_ordenadas)
{
  limpiarPantalla();
...
void buscar_por_ingredientes(Map *mapa_ingredientes)
{
  ...
  MapPair *par = map_search(mapa_ingredientes, ingrediente_actual);
    ...
    menu_recetas_favoritas(recetas_encontradas);

    char opcion;
    printf("¿Desea buscar recetas con otros ingredientes? (S/N): "); getchar();
    scanf("%c", &opcion);
    if(opcion == 's' || opcion == 'S') buscar_por_ingredientes(mapa_ingredientes);
}
...
int main() {
    ...
    do {
        ...
        case '2':
            ...
            break;
        case '3':
            buscar_recetas_posibles(recetas_ordenadas);
            break;
        case '4':
